...
POSTAGS="$POSTAGS PDS+PIAT+PIDAT+PIS PPER+PPER PPOSAT+PPOSS PRELS+PRF"
			...
			if [ $CNTSUM -gt 0 ] && [ $WORDCNTSUM -gt 0 ] ; then
				...
				for i in $FILE ; do
					if [[ $i =~ ^.*\|.*\|.*\|.*\|(.*)\|(.*)$ ]]; then
						...
						echo "${i}|${CNTPERCENT}|${WORDPERCENT}" >> $OUTPUT_DIR/output_typetoken_lemma_summary_${DAYMIN}_${DAYMAX}_${n}.csv.tmp
					fi
				done
				echo "----|----|----|----|----|----|----|----" >> $OUTPUT_DIR/output_typetoken_lemma_summary_${DAYMIN}_${DAYMAX}_${n}.csv.tmp
				echo "||||${CNTSUM}|${WORDCNTSUM}|100,00000|100,00000" >> $OUTPUT_DIR/output_typetoken_lemma_summary_${DAYMIN}_${DAYMAX}_${n}.csv.tmp
				mv $OUTPUT_DIR/output_typetoken_lemma_summary_${DAYMIN}_${DAYMAX}_${n}.csv.tmp $OUTPUT_DIR/output_typetoken_lemma_summary_${DAYMIN}_${DAYMAX}_${n}.csv
			fi
		fi
	fi
done
